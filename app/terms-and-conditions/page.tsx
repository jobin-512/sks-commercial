...
const TermsAndConditions = () => (
  <main className={`max-w-3xl mx-auto px-4 py-12 ${roboto.className}`}>
    ...
    <section className="mb-6">
      <p>Welcome to SKS Tech Solution. When you visit or use our website, you agree to be bound by these terms and conditions. Please read carefully because by continuing to browse the site, you automatically agree to everything mentioned here. If you disagree, then unfortunately, you should discontinue using the website or any of our services on an immediate basis.</p>
    </section>
    <section className="mb-6">
      <h2 className="text-xl font-semibold mb-2">Client Responsibilities & Service Limitations</h2>
      <p>Before services can begin, it's assumed that the client doesn't have duplicate websites or repeated pages, and no sneaky redirect tricks or doorway pages. The client also confirms they have not exchanged links with link farms or been involved in spamming methods that can mess up the website's ranking with Google and other search engines. Please note that we can't guarantee exact results or promise a 100% improvement in traffic or sales, because search engines are unpredictable and every project is unique in its own way. All service fees are required in advance and, unfortunately, are not refundable. If the client alters the pages that SKS Tech Solution has already optimized, or if they ignore recommended changes, any form of guarantee will be cancelled immediately. SKS Tech Solution sticks to only ethical SEO practices. It will nullify any agreement or guarantee if it turns out that the client has been doing things search engines consider bad, like using hidden links, link schemes, or any kind of page cloaking or automated submissions, even without telling us. Also, SKS Tech Solution has the right to use client websites, designs, and layouts for promotional purposes, like adding to portfolio pages or case studies, and even adding subtle links in the footer unless the client clearly asks us not to.</p>
    </section>
    <section className="mb-6">
      <h2 className="text-xl font-semibold mb-2">Grant of Specific Legal Rights</h2>
      <p>To do proper SEO, we may require access to your site's backend, i.e., FTP and admin panels. We may also need to contact your web developer or third-party provider if required. Full access to your current website analytics is expected so we can monitor and measure performance. Clients also give permission for SKS Tech Solution to use their logos, trademarks, and website images to create content or articles needed for SEO strategy. If the client's website lacks enough written content, then they are expected to send more in digital form. Otherwise, SKS Tech Solution can provide content at an additional cost, and that cost will be discussed beforehand. We can also write fresh content if needed, but again, it's something that's charged separately.</p>
    </section>
    <section className="mb-6">
      <h2 className="text-xl font-semibold mb-2">Non-Disclosure Agreement</h2>
      <p>During the course of the project and even after that, the client undertakes to strictly adhere to confidentiality terms and not disclose or use any confidential information provided by SKS Tech Solution. Also, we request that clients don't publicly criticise the company, employees, or partners on blogs, forums, or any social media during or even after project completion. This includes avoiding negative discussions or asking for SEO help in a way that makes SKS Tech Solution look bad. If this clause is violated, the client may be liable to pay damages, the amount of which would be based on the harm caused. It's just about keeping mutual respect and professionalism.</p>
    </section>
    <section className="mb-6">
